...
export class AccessControlAgent implements AttackAgent {
  ...
  analyze(input: ContractAnalysis): AttackFinding[] {
    ...
    const privileged = input.functions.filter((name) =>
      PRIVILEGED_FUNCTIONS.includes(name),
    );
    ...
    if (privileged.length > 0 && !hasExplicitGuard) {
      findings.push({
        ...
        confidence: "high",
        description:
          "Privileged state-changing functions were detected without strong access control signals.",
        attackVector:
          "An unprivileged caller invokes admin-like functions and mutates protected state.",
        suggestedTest:
          "Call the privileged function from an arbitrary address and assert that the mutation succeeds unexpectedly.",
    ...
    if (input.source.includes("tx.origin")) {
      findings.push({
        ...
        confidence: "medium",
        description:
          "Authorization appears to depend on tx.origin, which is unsafe and phishing-prone.",
        attackVector:
          "An attacker routes the victim through an intermediate contract so tx.origin stays privileged.",
        suggestedTest:
          "Invoke the protected action through an attacker contract and assert that origin-based authorization still passes.",
        contract: input.contractName,
