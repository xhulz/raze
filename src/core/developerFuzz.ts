 ...
 */
function chooseObservableState(
  source: string,
  ...
  if (
    ...
    return variables.find(
      (variable) =>
        variable.type.startsWith("mapping") && variable.keyType === "address",
    );
  }
  return (
    variables.find((variable) => variable.type.startsWith("uint")) ??
    variables.find((variable) => variable.type.startsWith("mapping"))
  );
}
...
function buildFamilyPlan(
  ...
  if (
    signature.paramTypes.some(
      (type) => type.startsWith("uint") || type.startsWith("int"),
    )
  ) {
    plans.push({
      contractName,
...
`;
  ...
  if (plan.family === "state-transition") {
    ...
    const observedExpr = buildObservedExpression(observable, signature);
    const directionCheck = ["decrement", "burn", "withdraw"].includes(
      signature.name,
    )
...
export async function generateDeveloperFuzzTests(
  ...
  const selectedSignatures = input.functionSelector
    ? signatures.filter(
        (signature) => signature.name === input.functionSelector,
      )
    : signatures;
  const plans = selectedSignatures.flatMap((signature) =>
    buildFamilyPlan(analysis.contractName, signature, analysis.source),
  );

  const skippedFunctions = selectedSignatures
    .filter(
      (signature) =>
        !plans.some((plan) => plan.functionName === signature.name),
    )
  ...
  for (const signature of selectedSignatures) {
    const functionPlans = plans.filter(
      (plan) => plan.functionName === signature.name,
    );
    ...
    const relativeImport = path
      .relative(path.dirname(testFilePath), analysis.contractPath)
      .replace(/\\/g, "/");
