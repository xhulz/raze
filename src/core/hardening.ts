 ...
 */
export function suggestionsFromFindings(
  ...
  for (const finding of findings) {
    if (finding.type === "access-control") {
      suggestions.set("access-control", {
        ...
        whyItMatters:
          "Privileged state-changing paths that stay publicly callable can let arbitrary actors mutate protected state.",
        recommendedChange:
          "Add an explicit authorization check or role gate before the privileged mutation path executes.",
    ...
    if (finding.type === "arithmetic") {
      suggestions.set("arithmetic", {
        ...
        issue: finding.description,
        whyItMatters:
          "Unchecked or weakly constrained arithmetic can wrap, drift, or silently violate state assumptions.",
        recommendedChange:
          "Replace unchecked arithmetic with explicit bounds checks or safer accounting logic for the affected mutation path.",
        confidence: finding.confidence,
        behaviorChange: true,
    ...
    if (finding.type === "reentrancy") {
      suggestions.set("reentrancy", {
        ...
        issue: finding.description,
        whyItMatters:
          "External calls before internal state finalization can allow reentrant callbacks to reuse the vulnerable path.",
        recommendedChange:
          "Apply checks-effects-interactions ordering or an explicit reentrancy guard before the external call occurs.",
        confidence: finding.confidence,
        behaviorChange: true,
    ...
    if (finding.type === "flash-loan") {
      suggestions.set("flash-loan", {
        ...
        issue: finding.description,
        whyItMatters:
          "Flash loan callbacks execute arbitrary logic inside a single transaction. Without invariant checks, an attacker can borrow large sums, distort contract state, and repay before the transaction reverts.",
        recommendedChange:
          "Record the contract's token balance or critical reserve state before delegating to the callback. Assert the invariant holds after the callback returns and before accepting repayment. Apply nonReentrant to all callback entry points.",
        confidence: finding.confidence,
        behaviorChange: true,
    ...
    if (finding.type === "price-manipulation") {
      suggestions.set("price-manipulation", {
        title:
          "Replace spot price reads with TWAP or Chainlink feed with staleness check",
        issue: finding.description,
        whyItMatters:
          "Single-block AMM spot prices can be moved by a large swap in the same transaction, enabling attackers to feed manipulated prices to dependent logic — especially when combined with a flash loan.",
        recommendedChange:
          "Use a TWAP oracle (Uniswap V3 observe(), or a custom accumulator) with a minimum observation window of at least 15 minutes. If using Chainlink, validate updatedAt against block.timestamp with a staleness threshold. Never trust a single getReserves() call as a price source.",
        confidence: finding.confidence,
        behaviorChange: true,
...
export async function suggestHardening(
  ...
  if (
    analysis.riskSignals.includes("low-level-call") &&
    ...
    suggestions.push({
      ...
      whyItMatters:
        "Low-level calls can widen attack surface and make failure behavior less obvious if not carefully ordered.",
      recommendedChange:
        "Document expected failure behavior and make sure external calls happen after critical internal effects when possible.",
      confidence: "medium",
      behaviorChange: false,
      followUpTest:
        "Add a regression test proving failed external calls do not leave partial state updates behind.",
    });
  }
