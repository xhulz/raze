 ...
 */
export async function runAttackPipeline(
  input: AttackPipelineInput,
): Promise<AttackPipelineResult> {
  ...
  const validatedPlans = input.attackPlan
    ? [
        (await validateAttackPlan(input, input.attackPlan, "ai-authored"))
          .validatedPlan,
      ]
    : await Promise.all(
        (await deriveFallbackPlans(analysis, findings)).map(
          async (plan) =>
            (await validateAttackPlan(input, plan, "heuristic-fallback"))
              .validatedPlan,
        ),
      );
  const generatedTests = await generateProofScaffolds(
    input.projectRoot,
    validatedPlans,
  );
  ...
  if (allSolFiles.length > 1) {
    ...
    crossContractFindings = deriveCrossContractFindings(analyses, graph).filter(
      (f) =>
        f.callerContract === analysis.contractName ||
        f.calleeContract === analysis.contractName,
    );
  }
