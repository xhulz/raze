...
export const validatedPlanSchema = attackPlanSchema.extend({
  ...
  normalizedSampleArguments: z.array(
...
export const reportWriteSchema = z.object({
  ...
  analysisSource: z
    .enum(["heuristic", "ai-orchestrated"])
    .default("ai-orchestrated"),
  hypothesisStatus: z
    .enum(["none", "ai-proposed", "validated"])
    .default("validated"),
  proofStatus: z
    .enum(["no-scaffold", "scaffold-generated", "executed"])
    .default("scaffold-generated"),
  confirmationStatus: z
    ...
    .default("validated-plan"),
  decision: z
    .enum(["fix-now", "investigate", "review", "no-action"])
    .default("investigate"),
  decisionReason: z
    .string()
    .min(1)
    .default("Review the structured result before deciding on remediation."),
