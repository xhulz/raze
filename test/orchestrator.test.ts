...
test("generateProofScaffolds materializes an access-control proof with observable mutation assertions", async () => {
  ...
  const { validatedPlan } = await validateAttackPlan(
    ...
    },
    {
      ...
      proofGoal:
        "Show that an unprivileged caller mutates balances without a revert.",
  ...
  assert.match(
    generated.source,
    /uint256 beforeValue = uint256\(target\.balances\(0x000000000000000000000000000000000000BEEF\)\);/,
  );
  assert.match(generated.source, /vm\.prank\(address\(0xDEAD\)\);/);
  assert.match(
    generated.source,
    /target\.mint\(0x000000000000000000000000000000000000BEEF, 5\);/,
  );
  assert.match(
    generated.source,
...
test("runAttackPipeline can confirm reentrancy when the scaffold proves reentry and extracted value", async () => {
  ...
  const result = await runAttackPipeline({
    ...
    attackPlan: {
      ...
      expectedOutcome:
        "The generated scaffold should exercise the reentrancy setup path.",
  ...
  assert.match(
    result.assessment.decisionReason,
    /reproduced the unsafe behavior/i,
  );
  assert.match(
    result.generatedTests[0]?.source ?? "",
    /reentrant callback was not observed/,
  );
  assert.match(
    result.generatedTests[0]?.source ?? "",
    /attacker did not extract excess value/,
  );
  assert.match(
    result.assessment.interpretation,
    /execution-backed confirmation/i,
  );
});

test("generateProofScaffolds uses lender scaffold for contracts that expose flashLoan()", async () => {
  const tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "raze-flash-lender-"),
  );
  await fs.cp(path.join(fixturesRoot, "flash-loan-lender"), tmpRoot, {
    recursive: true,
  });
  ...
  const { validatedPlan } = await validateAttackPlan(
    ...
    },
    {
      ...
      attackHypothesis:
        "flashLoan updates totalDeposits before the callback with no invariant check.",
