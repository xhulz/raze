...
test("discoverContracts finds solidity files in a Foundry project", async () => {
  const contracts = await discoverContracts(
    path.join(fixturesRoot, "reentrancy"),
  );
...
test("analyzeContract selects flash-loan agent for flash-loan fixture", async () => {
  ...
  assert.ok(
    analysis.recommendedAgents.includes("flash-loan"),
    `expected flash-loan in ${analysis.recommendedAgents.join(", ")}`,
  );
});
...
test("flash-loan agent returns findings for flash-loan fixture", async () => {
  ...
  const findings = runAttackAgents(analysis);
  assert.ok(
    findings.some((f) => f.type === "flash-loan"),
    `expected flash-loan finding, got: ${findings.map((f) => f.type).join(", ")}`,
  );
...
test("analyzeContract selects price-manipulation agent for price-manipulation fixture", async () => {
  ...
  assert.ok(
    analysis.recommendedAgents.includes("price-manipulation"),
    `expected price-manipulation in ${analysis.recommendedAgents.join(", ")}`,
  );
});
...
test("price-manipulation agent returns findings for price-manipulation fixture", async () => {
  ...
  const findings = runAttackAgents(analysis);
  assert.ok(
    findings.some((f) => f.type === "price-manipulation"),
    `expected price-manipulation finding, got: ${findings.map((f) => f.type).join(", ")}`,
  );
...
test("generateProofScaffolds writes deterministic Foundry tests", async () => {
  ...
  await fs.cp(path.join(fixturesRoot, "arithmetic"), tmpRoot, {
    recursive: true,
  });
  const analysis = await analyzeContract({
    projectRoot: tmpRoot,
  ...
  const validatedPlans = await Promise.all(
    fallbackPlans.map(
      async (plan) =>
        (
          await validateAttackPlan(
            { projectRoot: tmpRoot, executionContext: "cli" },
            plan,
            "heuristic-fallback",
          )
        ).validatedPlan,
...
test("runAttackPipeline produces a report", async () => {
  ...
  await fs.cp(path.join(fixturesRoot, "reentrancy"), tmpRoot, {
    recursive: true,
  });
...
test("reentrancy scaffold includes regression test function", async () => {
  const tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "raze-reentrancy-regression-"),
  );
  await fs.cp(path.join(fixturesRoot, "reentrancy"), tmpRoot, {
    recursive: true,
  });
  const result = await runAttackPipeline({
    ...
    attackPlan: {
      ...
      proofGoal:
        "Show that a callback can revisit withdraw before the balance is zeroed.",
      expectedOutcome:
        "Attacker callback reaches the target function multiple times.",
...
test("cross-contract findings upgrade decision from no-action to review", async () => {
  const tmpRoot = await fs.mkdtemp(
    path.join(os.tmpdir(), "raze-cross-contract-"),
  );
  await fs.cp(path.join(fixturesRoot, "cross-contract"), tmpRoot, {
    recursive: true,
  });
  const result = await runAttackPipeline({
    projectRoot: tmpRoot,
  ...
  assert.ok(
    result.assessment.decision === "review" ||
      result.assessment.decision === "investigate" ||
      result.assessment.decision === "fix-now",
    `expected decision >= review, got: ${result.assessment.decision}`,
  );
  assert.ok(
    (result.crossContractFindings?.length ?? 0) > 0,
    "expected cross-contract findings",
  );
});
